...
- 支持从一句话故事点直接批量产出若干集可播放 mp4 样片，用于制作人验收节奏、分集结构和导出链路。
...
- 本地 deterministic 样片 runtime 不替代正式图像/视频模型，它是用于低成本验收流程、测试导出和恢复生产状态的兜底产出路径。
...
- API：`POST /film/auto-drama/produce-videos`。
- CLI：`python3 scripts/produce_auto_drama_videos.py`。

用户输入：

- 系列标题。
- 一句话故事梗概。
- 目标集数，默认 3 集，最多沿用 Auto Drama 的 12 集限制。
- 单镜头时长、分辨率和是否保留中间镜头文件。

系统行为：

- 先复用 `AutoDramaPipeline` 生成 `novel_plan` 和 `episode_packages`。
- 每集基于生产包生成 3 个镜头短片，并用 FFmpeg 合成为一个可播放 mp4。
- 每集输出独立 `episode_manifest.json`，记录集数、脚本、分镜、资产数量、镜头文件和最终 mp4 路径。
- 系列输出 `series_manifest.json`，汇总所有集的产物路径、运行配置和 QA 摘要。
- 默认不提交 `output/` 产物，避免把大文件和临时资产写入 Git。

验收：

- API 返回 `status=completed`、`series_manifest_url` 和每集 `video_url`。
- CLI 输出 manifest 路径，并在 `output/productions/{run_id}/` 下生成至少 N 个 mp4。
- `ffprobe` 能读取每个 mp4 的时长和视频流信息。
- 缺少 FFmpeg 时返回可理解错误，不破坏已生成的 Auto Drama 结构化结果。
...
- `POST /film/auto-drama/produce-videos`：生成多集本地可播放样片视频和 manifests。
...
| 多集视频产出 | `test_episode_video_producer.py`、API 冒烟、`ffprobe` | 运行 CLI/API 后打开 output mp4 |
...
- 本地样片视频只验证剧情结构、节奏、字幕信息和导出可播放性；商业成片需要替换镜头 runtime 并接入真实图像、视频、配音与音乐资产。
