...
AIDrama 是面向工业化 AI 漫剧/AI Film Engine 的内容试制工作区。最终主平台基座采用 Starter Kit 指定的 Jellyfish；当前仓库可运行入口已统一为 AIDrama Studio，并针对本机环境补了 Docker 启动、端口映射、密钥引导和项目级文档。
...
- `AIDrama Studio`：当前仓库内可运行入口，用于本地试制、DashScope/Wanx 验证和迁移期回归测试，按 Jellyfish-oriented Studio OS 方向继续演进。
...
- `src/film_engine/` 提供工业化 Film Core 最小闭环：Story Graph、Director Planner、Runtime、Director DSL、Shot Graph、Prompt Compiler、Registry、QA、Retry、Film State、Generation Ledger、Batch Production Runner、Final Editing
...
4. 同一镜头生成 3-6 个候选，记录 prompt、参考图、模型、成本和人工评分。
   Film Core 的 `GenerationLedger` 已用于沉淀每次 runtime attempt、QA 结果、重试决策、输出 URI 和人工复盘信息。
5. 多个 Director Program 进入 `BatchProductionPlan`，由 `BatchProductionRunner` 按优先级、上限和错误隔离策略串行跑批，并汇总每个序列的 QA、Retry、Ledger、成本与输出。
6. 样片发布到抖音、快手、B 站、小红书或 YouTube Shorts，用完播和互动数据反推题材。
7. 胜出题材扩到 5 集，再找网文作者、MCN、小版权方做 B2B 试播集或联合制作。

当前仓库已提供 D7 验证模板：

- `samples/pilot_samples/three_60_90s_pilots.yaml`：3 个 60-90 秒题材样片方向。
- `samples/series_production/vertical_suspense_5ep.yaml`：1 个 5 集竖屏悬疑短漫剧蓝图，可通过 `SeriesProductionPlanner` 编译为批量生产计划并 dry-run 验证。
