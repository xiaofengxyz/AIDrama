...
AIDrama 是 AI 漫剧内容试制工作区，当前采用 LumenX Studio 作为主基座，并针对本机环境补了 Docker 启动、端口映射、密钥引导和项目级文档。

项目的第一目标是帮助小团队把内容生产链路跑顺：少花时间在工具切换、手工记录、素材丢失和重复抽卡上，多花时间验证题材、角色、爽点和发布数据。
...
- 不把上游源码克隆目录提交进主仓库。`external/open-source-repos/lumenx` 只作为本机参考源。
...
- `.env`：由 `scripts/bootstrap_env.sh` 从 `Doc/accounts` 生成，已忽略提交。
