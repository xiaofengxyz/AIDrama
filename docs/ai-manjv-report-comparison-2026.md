...
- 当前 AIDrama/LumenX 工程代码

## 核心结论

报告对行业瓶颈的判断是正确的：角色一致性、镜头可控、长剧情记忆、自动 QA、自动重试，才是 AI 漫剧工业化的核心壁垒。

但报告给出的“直接切换 huobao-drama 为主框架”不适合当前仓库阶段。当前工程已经完成 LumenX 基座适配、Docker 端口、DashScope-first 路由、系列资产和前后端工作台，继续换基座会把工作重心从内容生产闭环转移到平台迁移。
...
| 维度 | 报告观点 | 当前仓库事实 | 本轮决策 |
|---|---|---|---|
| 主基座 | 推荐 `huobao-drama` | 本仓库已经以 LumenX 作为主工程，前后端、Docker、provider 路由已经可运行 | 不切换基座，吸收 huobao 的 runtime/后期思路 |
| LumenX 评价 | 偏企业平台，小团队适配较弱 | LumenX 官方定位就是短漫剧一站式生产，覆盖资产、风格、分镜、视频和合成；本仓库已降低部署门槛 | 继续作为当前最小团队试制基座 |
| Toonflow | Agent/无限画布/Skill 成熟 | Toonflow 更像重型桌面创作 IDE，适合参考 Agent 组织和画布生产 | 暂不迁移，吸收 Skill 文件化和记忆思路 |
| Jellyfish | Starter Kit 推荐为 Studio OS | Jellyfish 与“短剧生产工作区”方向贴近，但当前仓库未接入 | 作为后续产品形态参考，不在本轮引入 |
...
3. `Jellyfish` 很符合 Studio OS 方向，但当前仓库已有 LumenX 工作台，短期更适合把 Film Core 做成可移植内核，而不是换壳。
...
1. 保留 LumenX 作为可运行生产工作台。
2. 删除/隔离 external 下的重复参考项目，避免双配置干扰。
3. 新增独立 Film Core，把工业化能力做成可测试、可替换、可迁移的内核。
4. 继续用镜头级台账、QA、Retry 和 Film State 降低抽卡随机性。
